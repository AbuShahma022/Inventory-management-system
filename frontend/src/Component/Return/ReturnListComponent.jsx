...
function ReturnListComponent() {
  ...
  const { list, listTotal } = useSelector((state) => state.return);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-5 gap-3">
        <h2 className="text-2xl font-semibold text-gray-800">Return List</h2>
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {list.length > 0 ? (
              list.map((item, index) => (
                <tr key={item._id} className="hover:bg-gray-50 transition-colors text-gray-800">
                  <td className="p-3 border-b">{index + 1 + (pageNo - 1) * perPage}</td>
                  <td className="p-3 border-b font-medium">{item.Customers?.[0]?.Name || "—"}</td>
                  <td className="p-3 border-b">{item.GrandTotal}</td>
                  <td className="p-3 border-b">{item.ShippingCost}</td>
                  <td className="p-3 border-b">{item.VatTax}</td>
                  <td className="p-3 border-b">{item.OtherCost}</td>
                  <td className="p-3 border-b">{item.Discount}</td>
                  <td className="p-3 border-b">{dayjs(item.CreatedDate).format("DD MMM YYYY")}</td>
              ...
              <tr>
                <td colSpan="9" className="text-center text-gray-500 py-6">No return records found</td>
              </tr>
            )}
          </tbody>
        </table>
...
export default ReturnListComponent;
