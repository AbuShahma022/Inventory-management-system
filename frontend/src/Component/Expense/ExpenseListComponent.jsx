...
import { ExpenseList, ExpenseDelete } from "../../APIRequest/ExpenseAPI.js";
...
function ExpenseListComponent() {
  ...
  const { list, listTotal } = useSelector((state) => state.expense);
  ...
  const handleDelete = async(id) => {
    ...
    if (result.isConfirmed) {
      let success = await ExpenseDelete(id);
      if (success === true) {
        await ExpenseList(pageNo, perPage, searchKeyword);
      }
    }
  };
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-100 text-gray-700">
            <tr>
              ...
              <th className="p-3 border-b">Expense Type</th>
            ...
            {list.length > 0 ? (
              list.map((expense, index) => (
                ...
                >
                  ...
                  <td className="p-3 border-b">{expense.Note}</td>
                  <td className="p-3 border-b">
                    {dayjs(expense.CreatedDate).format("DD MMM YYYY")}
                  </td>
                  <td className="p-3 border-b text-center">
                    <div className="flex justify-center gap-3 text-lg">
                      <Link
                        to={`/expense/create_update/${expense._id}`}
