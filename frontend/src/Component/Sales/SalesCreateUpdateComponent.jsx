...
import { SalesCreate, SalesDetailsById } from "../../APIRequest/SalesAPI";
...
function SalesCreateUpdateComponent() {
  const { id } = useParams();
  ...
  // If has id → details mode
  useEffect(() => {
    if (id) {
      (async () => {
        const data = await SalesDetailsById(id);
        if (data) {
          setDetailsData(data);
        }
      })();
  ...
  const removeRow = (i) => {
    ...
    setChild(child.filter((_, idx) => idx !== i));
  };

  const handleSubmit = async () => {
    if (IsEmpty(parent.CustomerId)) return ErrorToast("Customer Required");
    if (child.some((c) => IsEmpty(c.ProductId))) return ErrorToast("Product Required");

    const res = await SalesCreate({ Parent: parent, Child: child });
  ...
  if (id && detailsData) {
    const customerName =
      customers.find((c) => c._id === detailsData.CustomerId)?.Name || "Unknown Customer";
  ...
  return (
    <div className="max-w-3xl mx-auto mt-8 p-6 bg-white rounded-2xl shadow">
      ...
      {child.map((row, i) => (
        <div
          key={i}
          className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-3 border p-3 rounded-lg"
        >
          <select
          ...
          <input
            ...
            className="border px-2 py-2 rounded"
            value={row.Qty}
            onChange={(e) => handleChildChange(i, "Qty", e.target.value)}
          />
          ...
          <input
            ...
            className="border px-2 py-2 rounded"
            value={row.UnitCost}
            onChange={(e) => handleChildChange(i, "UnitCost", e.target.value)}
          />
...
export default SalesCreateUpdateComponent;
